...
#Tasks:
#   Run Q4 100 times (each time using a randomly generated value for the country code) and record the average query time required for the database to answer Q4 queries using each version of table Parts.
#   Define and create one INDEX with the goal of optimizing query Q4 (take note of its space cost).
#   Repeat Task 9, i.e., Run Q4 100 times and record the average query time required for the database to answer Q4 queries using each version of table Parts.
#   Compare and contrast the results obtained in Tasks 9 and 10. Discuss in which cases the space cost of the index seemed like a worthwhile "investment.”
...
def main():
    global connection, cursor
    print("executing part 3, task 1")
    loadCountries()
    loadUPC()
    runQuery()
    print("Executing Part 3, task 2")
    runOptimizeindex()
